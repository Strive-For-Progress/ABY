...
int32_t test_hmac_sha1_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl, uint32_t nvals, uint32_t nthreads, e_mt_gen_alg mt_alg, e_sharing sharing) {
	...
	// printf("%d\n",sha1bits_per_party * nvals);

	// uint32_t testvec = 0x01;
	...
	// msgS.SetBits((uint8_t*) &testvec, 0, 32);
	...
	share *s_msgS, *s_msgSi, *s_msgSo, *s_msgC, *s_hash_out;
	...
	s_hash_out = BuildHMACSHA1Circuit(s_msgS, s_msgSi, s_msgSo, s_msgC, msgS.GetArr(), msgSi.GetArr(), msgSo.GetArr(), msgC.GetArr(), plain_out, nvals, (BooleanCircuit*) circ);
...
share* BuildHMACSHA1Circuit(share* s_msgS, share* s_msgSi, share* s_msgSo, share* s_msgC, uint8_t* msgS, uint8_t* msgSi, uint8_t* msgSo, uint8_t* msgC, uint8_t* plain_out, uint32_t nvals, BooleanCircuit* circ) {
...
init_variables(s_h, h, nvals, circ);
	...
	out = process_block(s_msg, msg+party_in_bytelen, tmp_plain_out, s_h, h, nvals, circ);
/////*//
